...
| `CADDY_DOMAIN_EXTRA_DOMAINS` | `null` | Additional complete site blocks appended after the main domain block. Useful to define extra domains or virtual hosts with their own configuration. Use the syntax `{env.XXX}` instead of `{$XXX}` for environment variables. **Tip:** To provide multiline values in docker-compose.yml, use `\|`. Example: <br>`CADDY_DOMAIN_EXTRA_DOMAINS: \|`<br>&nbsp;&nbsp;`extra.domain.tld {`<br>&nbsp;&nbsp;&nbsp;&nbsp;`respond "Hello from extra domain!"`<br>&nbsp;&nbsp;`}` |
